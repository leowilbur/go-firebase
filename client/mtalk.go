...
// convertECDHToECDSA converts an ECDH private key to an ECDSA private key
func convertECDHToECDSA(ecdhKey *ecdh.PrivateKey) (*ecdsa.PrivateKey, error) {
	// Get the raw bytes of the private key
	keyBytes := ecdhKey.Bytes()

	// Create a big.Int from the key bytes
	...
	ecdsaKey.PublicKey.X, ecdsaKey.PublicKey.Y = elliptic.P256().ScalarBaseMult(keyBytes)
...
func NewMTalkCon(device *firebase_api.FirebaseDevice) (*MTalkCon, error) {
	...
	// Convert ECDH key to ECDSA key for ecego
	ecdsaKey, err := convertECDHToECDSA(privateKey)
	if err != nil {
		err = fmt.Errorf("convertECDHToECDSA: %w", err)
		return nil, err
	}
